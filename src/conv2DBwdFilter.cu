                               ...
                               int group)
{
    ...
    const int dim_w[4] = {output_c, input_c, kernel_h, kernel_w};

    CUDNN_CHECK(cudnnSetFilterNdDescriptor(wDesc, CUDNN_DATA_FLOAT, CUDNN_TENSOR_NCHW, 4, dim_w));

    const int dim_x[4] = {input_n, input_c, input_h, input_w};
    const int stride_x[4] = {input_c * input_h * input_w, input_h * input_w, input_w, 1};
    CUDNN_CHECK(cudnnSetTensorNdDescriptor(xDesc, CUDNN_DATA_FLOAT, 4, dim_x, stride_x));

    const int dim_y[4] = {input_n, output_c, output_h, output_w};
    const int stride_y[4] = {output_c * output_h * output_w, output_h * output_w, output_w, 1};
    CUDNN_CHECK(cudnnSetTensorNdDescriptor(yDesc, CUDNN_DATA_FLOAT, 4, dim_y, stride_y));
    ...
    int stride[] = {1, 1};
    int upscale[] = {1, 1};
    CUDNN_CHECK(cudnnSetConvolutionNdDescriptor(convDesc, 2, pad, stride, upscale, CUDNN_CROSS_CORRELATION, CUDNN_DATA_FLOAT));
